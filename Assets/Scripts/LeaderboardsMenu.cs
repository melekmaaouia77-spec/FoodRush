...
public class LeaderboardsMenu : Panel
{
    ...
    public async void AddScoreAsync(int score)
    {
        ...
        try
        {
            var playerEntry = await LeaderboardsService.Instance.AddPlayerScoreAsync("Players_Score", score);
    ...
    private async void LoadPlayers(int page)
    {
        ...
        try
        {
            ...
            var scores = await LeaderboardsService.Instance.GetScoresAsync("Players_Score", options);
            ClearPlayersList();
