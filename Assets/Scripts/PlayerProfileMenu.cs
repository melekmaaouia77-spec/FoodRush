...
public class PlayerProfileMenu : Panel
{
    ...
    private async void RemoveFriend()
    {
        ...
        try
        {
            ...
            ErrorMenu panel = (ErrorMenu)PanelManager.GetSingleton("error");
            panel.Open(ErrorMenu.Action.None, "players removed from your friends list successfully.", "OK");
        }
        catch
        {
            ErrorMenu panel = (ErrorMenu)PanelManager.GetSingleton("error");
            panel.Open(ErrorMenu.Action.None, "Failed to remove the players from your friends list.", "OK");
        }
